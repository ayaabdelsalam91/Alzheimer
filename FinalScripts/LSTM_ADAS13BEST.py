...
def train_lstm(TargetName ,X_, Y_,Trainseq_length, TestData , Testseq_length,RID, D3Data , D3seq_length,D3RID ,splits=None,
	learning_rate = 0.005,n_neurons=32, n_layers = 2 , alpha=0.15,n_epochs=350, DefaultValues = None  , isBaseline =None):
	print (TargetName)
	...
	if(splits is not None):
		...
		with tf.Session() as sess:
			init.run()
			for epoch in range(n_epochs):
				# print (epochs  ,  X_.shape,Y_.shape , Trainseq_length.shape)
				...
				interval_25 , interval_75 = get_interval(MAE_values)
				trueMAE = np.median(MAE_values)
				# print(trueMAE , interval_25 ,interval_75)
				interval_25,interval_75 = trueMAE-interval_25,interval_75-trueMAE
				# print(trueMAE , interval_25 ,interval_75)
			...
			for i in range(TestData.shape[0]):
					...
					for j in range(50):
						...
						if(seq[0]<20):
							TestData[i ,seq[0],0] =y_pred.flatten()[0]
			...
			for i in range(D3Data.shape[0]):
					...
					for j in range(50):
						...
						if(seq[0]<20):
							D3Data[i ,seq[0],0] =y_pred.flatten()[0]
							for ind in range(1,n_inputs):
