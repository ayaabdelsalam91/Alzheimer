...
def FillinDataForForecasting(TargetName,inputfile,outputfile,multivariate=False, shift=False,Testing = False):
	...
	if(not Testing):
		if( not multivariate):
			...
			for i in range (LSTMData.shape[0]):
				for j in range (1+numberOfFeatures,LSTMData.shape[1] ):
		...
		for i in range (LSTMData.shape[0]):
			...
			for j in range (1+numberOfFeatures,shiftingValue[i] ):
				if(np.isnan(LSTMData.values[i,j])):
					LSTMData.values[i,j] =  LSTMData.values[i,j-numberOfFeatures]
	...
	LSTMData.to_csv('/Users/aya/Documents/Research/Alzheimer/tadpole_challenge/ForecastProcessed_data/'+outputfile+'.csv',index=False)
...
def DuplicateData(DataType,TargetName , multivariate=False,shift=False,isnotDx=True):
	...
	data = '/Users/aya/Documents/Research/Alzheimer/tadpole_challenge/ForecastProcessed_data/LSTM'+DataType+'Data'+TargetName+'.csv'
	...
	print df.shape
	df.to_csv('/Users/aya/Documents/Research/Alzheimer/tadpole_challenge/ForecastProcessed_data/LSTMParsed'+DataType+'Data'+TargetName+'.csv',index=False)
...
def getLSTMInput(inputfile,outputfile,shift=False,Testing = False,DiffTarget=False , DiffFeatures=False):
	...
	if(not Testing):
		output = np.zeros((info.shape[0]*(NumberOfTimeSteps-1),info.shape[1]+1-numberOfFeatures))
		if(not DiffTarget):
			for i in range (info.shape[0]):
				for j in range (0,(NumberOfTimeSteps-1)):
					output[j+i*(NumberOfTimeSteps-1),0]=info[i,RID]
					for col in range(1 , (j+1)*numberOfFeatures+1):
	...
	df = DataFrame(output ,columns=cols)
	print df.shape
	df.to_csv('/Users/aya/Documents/Research/Alzheimer/tadpole_challenge/ForecastProcessed_data/'+outputfile+'.csv',index=False)
